    ...
    try{
      ...
      echo "error while sending email{$e}"
       office365ConnectorSend color: '#ff0000',message: " <h2>${imageName}-${env.JOB_NAME}: Veracode Scan upload  Failed </h2><h3>BUILD_NUMBER : ${env.BUILD_NUMBER}<br>  NODE_NAME  : ${env.NODE_NAME} ERROR_LOG: ${e} </b></h3>", status: 'FAILURE', webhookUrl:"${VeracodeUrl}"
          emailext body: "<h2>Error occurred during veracode sccan upload</h2>  <h3> Status code: 3 <br> ERROR_LOG:</b>${e} <br> BUILD_URL:${env.BUILD_URL}</h3>", subject: "${imageName}-${env.JOB_NAME} Veacode Scan upload  Failed ", from:'gajendrarok@gmail.com', to: 'gajendra-kt@dxc.com'
    }
  }
  stage("2nd stage "){
